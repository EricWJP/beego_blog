...
type Users struct {
	...
	IsAdmin         bool
	FollowedCount   int64
	MicropostsCount int64
	Comment         string    `orm:"type(longtext)"`
...
// no records exist
func GetAllUsers(query map[string]interface{}, fields []string, sortby []string, order []string,
...
func (c *Users) Authenticate(password string) bool {
	if c.Password == GetEncryptPassword(password) {
		return true
	}
	return false
}
...
func RSAEncrypt(orgidata []byte) ([]byte, error) {
	...
	//if byteSlice, err := rsa.EncryptPKCS1v15(rand.Reader, pub, orgidata); err != nil {
	//	return "", err
	//} else {
	//	md5str1 := fmt.Sprintf("%x", md5.Sum(byteSlice))
	//	return md5str1, nil
	//	//return hex.EncodeToString([]byte(md5str1)), nil
	//}
	return rsa.EncryptPKCS1v15(rand.Reader, pub, orgidata) //加密
}

func RSADecrypt(cipertext []byte) (string, error) {
	...
	if byteSlice, err := rsa.DecryptPKCS1v15(rand.Reader, priv, cipertext); err != nil {
		return "", err
	} else {
